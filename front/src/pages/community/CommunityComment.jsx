...
const CommentTxT = ({idx, content, createdAt, setDeleteComment, deleteComment}) => {
  ...
  const handleModify = async () => {
    ...
    try {
      const response = await request.put(`/community/comment/${id}/${idx}`, { content: modify.value });
      if(response.data === 1){
  ...
  return (
    <>
      {createdAt && !isInput ? <Txt idx={idx}>{modify.value},{timeAgo}</Txt> : 
      <input idx={idx} value={modify.value} onChange={modify.onChange}/>}
      {!isInput?<button type="button" onClick={()=>{setIsInput(true)}}>수정</button>:<button type="button" onClick={handleModify}>수정완료</button>}
  ...
  return (
    <CommentForm onSubmit={submitHandler}>
      {comments? comments.map((comment) => 
        <CommentTxT key={comment.id} idx={comment.id} content={comment.content} createdAt={comment.createdAt} setDeleteComment={
          setDeleteComment
        } deleteComment={deleteComment}/> 
      ) : <></>}
      <CommentInput>
        <ContentInput value={commentValue} onChange={(e)=>{setCommentValue(e.target.value)}} placeholder='댓글을 입력해주세요.'/>
        <CommentButton>
