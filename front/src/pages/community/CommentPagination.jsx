...
//paging
export const Pagination = ({id, setComments, setCurrentPage, pageNumbers, setPageNumbers, selectedPage, setSelectPage, totalComments}) => {
  ...
  return (
    <PageWrap> 
      ...
      ((pageNumbers[0] !== 1) ? 
      (<button type="button" style={{border:'none', cursor:'pointer', background:'white'}} onClick={() => { prevPage() }}><Icon icon="mdi:chevron-right" width="1.5rem" rotate={2} /></button>) : 
      null
      )}
    ...
    <button type='button' onClick={()=>{nextPage()}} style={{border:'none', cursor:'pointer' ,background:'white'}}> <Icon icon="mdi:chevron-right" width="1.5rem" /> </button>
    ) : <></> }
    </PageWrap>
  )
}